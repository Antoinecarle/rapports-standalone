...
export function mapToRapportSynthese(data: FusedRapportData) {
  ...
  console.log('[RapportDataMapper] Nom du logement:', {
    syntheseLogement: syntheseSection.logement,
    fullDataLogementName: data.fullData?.logementName,
    finalLogement: logement
  });
  ...
  if (data.fullData?.exitQuestion && data.fullData.exitQuestion.length > 0) {
    return data.fullData.exitQuestion.map((question, index) => {
      ...
      if (question.questionType === 'boolean') {
        ...
        console.log(`[RapportDataMapper] Exit question ${index}:`, {
