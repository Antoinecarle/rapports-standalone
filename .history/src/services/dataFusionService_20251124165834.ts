...
export class DataFusionService {
  ...
  private createDefaultAiData(
    ...
    allPieceIds.forEach(pieceId => {
      ...
      // Chercher d'abord dans etaperesponse avec le champ nom
      ...
      if (etapeWithName?.nom) {
        ...
        if (photoWithName?.nom) {
          ...
          if (rawPiece?.nom) {
            ...
            console.warn(`[DataFusionService] ⚠️ Le champ 'nom' n'est pas présent dans fullData pour ${pieceId}`);
            console.warn(`[DataFusionService] ⚠️ Veuillez ajouter le champ 'nom' dans l'endpoint rapportfulldata de Bubble.io`);
