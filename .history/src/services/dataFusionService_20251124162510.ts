...
export class DataFusionService {
  ...
  private createDefaultAiData(
    ...
    const reportMetadata: RapportDataJSON['reportMetadata'] = {
      ...
      dateDebut: rawData.parcours?.date_debut || dateStr,
      dateFin: rawData.parcours?.date_fin || dateStr,
      statut: 'Terminé',
      parcours: rawData.parcours?.parcours_id || rapportId,
    ...
    const syntheseSection: RapportDataJSON['syntheseSection'] = {
      logement: fullData.logementName || 'Logement',
      voyageur: rawData.agent?.nom || 'Voyageur',
      email: rawData.agent?.email || '',
      telephone: fullData.userPhone || rawData.agent?.telephone || '',
      dateDebut: rawData.parcours?.date_debut || dateStr,
      dateFin: rawData.parcours?.date_fin || dateStr,
      heureCheckin: rawData.timestamps?.checkin_start || '00:00:00',
      heureCheckout: rawData.timestamps?.checkout_start || '00:00:00',
      heureCheckinFin: rawData.timestamps?.checkin_end || '00:00:00',
      heureCheckoutFin: rawData.timestamps?.checkout_end || '00:00:00',
      noteGenerale: 8,
      sousNotes: {
        presenceObjets: 8,
        etatObjets: 8,
