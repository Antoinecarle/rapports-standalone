...
class FullDataService {
  ...
  async fetchFullData(rapportId: string): Promise<FullDataResponse> {
    try {
      ...
      // Problème 2: Clés dupliquées "logementName" - garder seulement la première occurrence
      ...
      if (logementNameMatch && logementNameMatch[1]) {
        const firstLogementName = logementNameMatch[1];
        // Supprimer toutes les occurrences de "logementName" sauf la première
        cleanedText = cleanedText.replace(/"logementName"\s*:\s*"[^"]*"/g, (match, offset) => {
          // Garder seulement la première occurrence
          return offset === logementNameMatch.index ? match : '';
        });
        console.warn('[FullDataService] ⚠️ Correction JSON appliquée: clés "logementName" dupliquées supprimées, valeur conservée:', firstLogementName);
      }
