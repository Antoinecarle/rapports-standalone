...
class FullDataService {
  ...
  async fetchFullData(rapportId: string): Promise<FullDataResponse> {
    try {
      ...
      // Problème: userPhone" au lieu de "userPhone"
      const jsonErrors = [
        { pattern: /userPhone"/g, replacement: '"userPhone"' },
        // Ajouter d'autres corrections si nécessaire
      ];

      let correctionApplied = false;
      jsonErrors.forEach(({ pattern, replacement }) => {
      ...
      if (correctionApplied) {
        console.warn('[FullDataService] ⚠️ Corrections JSON appliquées pour contourner les erreurs de l\'API Bubble');
      }
