...
export class DataFusionService {
  ...
  async loadAndFuseData(): Promise<FusedRapportData> {
    try {
      ...
      // rawData est optionnel - si elle échoue, on génère des données par défaut
  ...
  private createDefaultAiData(
    ...
    allPieceIds.forEach(pieceId => {
      ...
      // Trouver le nom de la pièce depuis fullData (etaperesponse ou photoPiececheckout)
      let pieceName = `Pièce ${pieceId.substring(0, 8)}`;

      // Chercher d'abord dans etaperesponse
      const etapeWithName = fullData.etaperesponse?.find(e => e.pieceid === pieceId && e.piecename);
      if (etapeWithName?.piecename) {
        pieceName = etapeWithName.piecename;
        console.log(`[DataFusionService] 📝 Nom de pièce trouvé dans etaperesponse: ${pieceName} pour ${pieceId}`);
      } else {
        // Sinon chercher dans photoPiececheckout
        const photoWithName = fullData.photoPiececheckout?.find(p => p.pieceid === pieceId && p.piecename);
        if (photoWithName?.piecename) {
          pieceName = photoWithName.piecename;
          console.log(`[DataFusionService] 📝 Nom de pièce trouvé dans photoPiececheckout: ${pieceName} pour ${pieceId}`);
        } else {
          ...
          if (rawPiece?.nom) {
            ...
            console.warn(`[DataFusionService] ⚠️ Aucun nom de pièce trouvé pour ${pieceId}, utilisation du nom par défaut: ${pieceName}`);
            // Log pour debug: afficher un exemple d'étape et de photo
            if (etapes.length > 0) {
              console.log(`[DataFusionService] 🔍 Exemple d'étape:`, etapes[0]);
            }
            if (photos.length > 0 && fullData.photoPiececheckout) {
              const photoExample = fullData.photoPiececheckout.find(p => p.pieceid === pieceId);
              console.log(`[DataFusionService] 🔍 Exemple de photo:`, photoExample);
            }
          }
        }
      }
