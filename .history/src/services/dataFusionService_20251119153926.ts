...
export class DataFusionService {
  /**
   * Nettoie le texte JSON en remplaçant les valeurs undefined par null
   * @param jsonText - Le texte JSON brut
   * @returns Le texte JSON nettoyé
   */
  private cleanJsonText(jsonText: string): string {
    // Remplacer toutes les occurrences de ": undefined," par ": null,"
    // et ": undefined}" par ": null}"
    return jsonText
  ...
  /**
   * Charge et fusionne les données de data.json et mydata.json
  ...
  async loadAndFuseData(): Promise<FusedRapportData> {
    try {
      ...
      const [aiDataResponse, rawDataResponse] = await Promise.all([
        fetch('/data.json'),
        fetch('/mydata.json')
      ]);
      ...
      if (!rawDataResponse.ok) {
        throw new Error(`Erreur lors du chargement de mydata.json: ${rawDataResponse.status}`);
      }

      // Récupérer le texte brut et nettoyer les undefined dans mydata.json
      const aiDataText = await aiDataResponse.text();
      const rawDataText = await rawDataResponse.text();

      const cleanedRawDataText = this.cleanJsonText(rawDataText);

      const aiData: RapportDataJSON = JSON.parse(aiDataText);
      const rawData: MyDataJSON = JSON.parse(cleanedRawDataText);

      // Récupérer les signalements enrichis depuis l'API Bubble
      const rapportId = aiData.reportMetadata.id;
      const bubbleSignalements = await signalementsService.fetchSignalements(rapportId);
  ...
  getPhotosForPiece(fusedData: FusedRapportData, pieceId: string): Etape[] {
    ...
    return etapes.filter(etape => etape.type === 'photo_taken');
  }
