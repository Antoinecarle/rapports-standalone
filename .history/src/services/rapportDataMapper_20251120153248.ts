...
export function mapToRapportSynthese(data: FusedRapportData) {
  ...
  return {
    ...
    voyageur: syntheseSection.voyageur,
    email: syntheseSection.email,
    telephone: syntheseSection.telephone,
    dateDebut: syntheseSection.dateDebut,
    dateFin: syntheseSection.dateFin,
...
export function mapToPiecesDetails(data: FusedRapportData) {
  return data.detailParPieceSection.map(piece => {
    ...
    // Extraire les URLs des photos capturées depuis mydata.json
    // Filtrer par etape_type "checkout" pour les photos de sortie
    // Note: On ne filtre plus par !is_todo car toutes les photos ont is_todo: true
    // La déduplication est déjà faite par getPhotosForPiece()
    ...
    // On enrichit les tâches existantes avec les photos des étapes correspondantes
    const tachesAvecPhotos = (piece.tachesValidees || []).map(tache => {
      // Si la tâche a un etapeId, chercher l'étape correspondante directement
      let tachePhoto = null;

      if (tache.etapeId) {
        // Nouvelle logique : chercher par etapeId
        // IMPORTANT : Il peut y avoir plusieurs étapes avec le même etape_id
        // On cherche celle qui a une photo (photo_url ou photo_base64)
        tachePhoto = etapes.find(etape => {
          if (etape.etape_id !== tache.etapeId) return false;

          // Vérifier si cette étape a une photo
          const hasPhoto = (etape.photo_url && etape.photo_url.trim() !== '') ||
                          (etape.photo_base64 && etape.photo_base64.trim() !== '');
          return hasPhoto;
        });
      } else {
        // Ancienne logique (fallback) : chercher par todo_title
      ...
      return {
        ...tache,
        photo_url: tachePhoto?.photo_url || tachePhoto?.photo_base64
      };
    });
 ...
 * Gère le cas où checkFinalSection est vide
 */
export function mapToCheckFinal(data: FusedRapportData) {
  // Si checkFinalSection est vide, retourner un tableau vide
