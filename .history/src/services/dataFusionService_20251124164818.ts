...
export class DataFusionService {
  ...
  private createDefaultAiData(
    ...
    allPieceIds.forEach(pieceId => {
      ...
      // Chercher d'abord dans etaperesponse avec le champ piecename
      ...
      if (etapeWithName?.piecename) {
        ...
        console.log(`[DataFusionService] 📝 Nom de pièce trouvé dans etaperesponse.piecename: ${pieceName} pour ${pieceId}`);
      } else {
        // Sinon chercher dans photoPiececheckout avec le champ piecename
        ...
        if (photoWithName?.piecename) {
          ...
          console.log(`[DataFusionService] 📝 Nom de pièce trouvé dans photoPiececheckout.piecename: ${pieceName} pour ${pieceId}`);
        } else {
          // Chercher dans rawData
          ...
          if (rawPiece?.nom) {
            ...
            console.warn(`[DataFusionService] ⚠️ Le champ 'piecename' n'est pas présent dans fullData pour ${pieceId}`);
            console.warn(`[DataFusionService] ⚠️ Veuillez ajouter le champ 'piecename' dans l'endpoint rapportfulldata de Bubble.io`);
