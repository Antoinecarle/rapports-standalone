    ...
    try {
      ...
      if (data.etaperesponse) {
        data.etaperesponse = data.etaperesponse.map((etape: EtapeResponse) => ({
          ...etape,
          referencephoto: etape.referencephoto?.startsWith('//')
            ? `https:${etape.referencephoto}`
            : etape.referencephoto,
          checkphoto: etape.checkphoto?.startsWith('//')
            ? `https:${etape.checkphoto}`
            : etape.checkphoto
        }));
      }
