...
export function mapToPiecesDetails(data: FusedRapportData) {
  return data.detailParPieceSection.map(piece => {
    ...
    const tachesAvecPhotos = (piece.tachesValidees || []).map(tache => {
      ...
      if (data.fullData?.etaperesponse && tache.etapeId) {
        const etapeFromFullData = data.fullData.etaperesponse.find(
          etape => etape.etapeid === tache.etapeId && etape.pieceid === piece.id
        );

        if (etapeFromFullData) {
          console.log(`[Mapper] Tâche "${tache.nom}" - etapeId: ${tache.etapeId}`);
          console.log(`[Mapper] - checkphoto: ${etapeFromFullData.checkphoto ? 'OUI' : 'NON'}`);
          console.log(`[Mapper] - referencephoto: ${etapeFromFullData.referencephoto ? 'OUI' : 'NON'}`);
